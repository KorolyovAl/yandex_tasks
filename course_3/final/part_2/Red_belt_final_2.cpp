...
void Test_1000() {
    ...
    LOG_DURATION("Test 22k") {
        TestFunctionality(doc, queries, expected);
    }
}
...
int main() {
    TestRunner tr;
    {
        LOG_DURATION("TestSerpFormat") {
            RUN_TEST(tr, TestSerpFormat); // 16 ms
        }
    }
    {
        LOG_DURATION("TestTop5") {
            RUN_TEST(tr, TestTop5); // 14 ms
        }
    }
    {
        LOG_DURATION("TestHitcount") {
            RUN_TEST(tr, TestHitcount); // 12 ms
        }
    }
    {
        LOG_DURATION("TestRanking") {
            RUN_TEST(tr, TestRanking); // 10 ms
        }
    }
    {
        LOG_DURATION("TestBasicSearch") {
            RUN_TEST(tr, TestBasicSearch); // 4 ms
        }
    }

    RUN_TEST(tr, Test_1000); // 4000 -> 3500 -> 2400
}
